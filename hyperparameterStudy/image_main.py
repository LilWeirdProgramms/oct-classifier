...
hyperparameter_list = Hyperparameter(
    ...
    repetition=["norm_im_lvl2"],
    residual=["residual"],
    mil=["mil"],
    crop=["crop", "cfalse"],  #cfalse
    normalize=["normalize"],  # nfalse
    image_type=["combined", "images", "structure"]
)
...
class ImageMain:
    ...
    def create_dataset(self, data_type="train"):
        ...
        if data_type == "train":
            ...
            self.class_weights = PreprocessData.calc_weights(self.ds_train)
    ...
    def train_model(self):
        logging.info(f"\n\nStarting calculation of {self.model_name}: \n\n")
        im = ImageModel(self.model_parameters)
        self.image_size = self.get_datasize()
        model = im.model(output_to=logging.info, input_shape=self.image_size)
        model.fit(self.ds_train.batch(8),
