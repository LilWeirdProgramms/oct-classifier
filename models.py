...
def classiRaw2D(input_size=(1536, 256, 1)):
    #dataformat: samples/A-scan x fast-axis x channels (unused)
    ...
    m = 1
    nconv = 3
    size_doutp = int32(np.floor(input_size[0]/2))
    
    dense1 = Permute((2, 3, 1))(inp) #dense layer connects input densely along last dimension
    dense1 = Dense(size_doutp, activation="relu", use_bias=False, kernel_initializer=init, bias_initializer=binit)(dense1)
    dense1 = Permute((3, 1, 2))(dense1)
    
    #create nconv downsampling layers
    conv = dense1
    for i in range(1, nconv+1):
        conv = Conv2D(32*m*i, 3, activation="relu", padding="same", kernel_initializer=init, bias_initializer=binit)(conv)
...
def classiRaw3D(input_size=(1536, 256, 256, 1)):
